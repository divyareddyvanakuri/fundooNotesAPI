...
# after login user can create note
...
class DisplayNote(GenericAPIView):
    ...
    def get(self,requset):
        try:
            ...
            q = Note.objects.filter(collaborator__in=[self.request.user.id])
        except Note.DoesNotExist:
            return Response(status=status.HTTP_404_NOT_FOUND)
        serializer1 = DisplayNoteSerializer(note,many=True)
        serializer = DisplayNoteSerializer(q,many=True)
        print(q)
        lists = [serializer1.data,serializer.data]
...
class UpdateNote(GenericAPIView):
    ...
    def get(self,request,pk):
        try:
            ...
            return Response(status=status.HTTP_404_NOT_FOUND)
        if (Note.objects.filter(user_id=request.user.id).count() == 2 or
                note.collaborator.filter(id=request.user.id).count() == 1):
            serializer = DisplayNoteSerializer(note)
            return Response(serializer.data)
        return Response(status=status.HTTP_404_NOT_FOUND)
    def put(self,request,pk):
        try:
            note = Note.objects.get(pk=pk,trash=False)
        except Note.DoesNotExist:
            return Response(status=status.HTTP_404_NOT_FOUND)
        # if note.trash == True:
...
class Remainder(GenericAPIView):
    ...
    def get(self, request, pk):
        try:
            ...
            return Response(status=status.HTTP_404_NOT_FOUND)
        serializer = DisplayNoteSerializer(note, many=True)
        note = Note.objects.get(user_id=request.user.id, pk=pk)
